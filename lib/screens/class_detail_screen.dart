...
class _ClassDetailScreenState extends ConsumerState<ClassDetailScreen> {
  ...
  Future<void> _joinStandbyQueue(GymClass gClass) async {
    ...
    try {
      ...
      final message = result.joinedNow
          ? 'You joined the standby queue at position ${result.position}.'
          : 'You are already in the standby queue at position ${result.position}.';
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Standby request failed: $message')),
      );
    } finally {
      if (mounted) {
        setState(() => _isLoading = false);
  ...
  Future<void> _leaveStandbyQueue(GymClass gClass) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('You left the standby queue.')),
      );
    } catch (e) {
      if (!mounted) return;
      var message = e.toString();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Unable to leave standby queue: $message')),
      );
    } finally {
      if (mounted) {
        setState(() => _isLoading = false);
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<GymClass?>(
      ...
      builder: (context, snapshot) {
        ...
        final isRegistered = reservations.any(
          (r) => r.id == gClass.id && r.status.toUpperCase() != 'STANDBY',
        );
        ...
        return StreamBuilder<int?>(
          ...
          builder: (context, queueSnapshot) {
            ...
            final statusTextColor = isRegistered
                            ...
                            child: SingleChildScrollView(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (gClass.standbyQueue.isNotEmpty) ...[
                                    const SizedBox(height: 8),
                                    Text(
                                      '${gClass.standbyQueue.length} in standby queue',
                                      style: textTheme.bodySmall?.copyWith(
                                        color: colorScheme.onSurfaceVariant,
                                        fontSize: 14,
                                      ),
