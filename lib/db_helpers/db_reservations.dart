...
class DBReservations {
  ...
  static Future<void> sendGroupInvites({
    ...
    String? hostMatNumber,
  }) async {
    if (inviteeUids.isEmpty) {
      return;
    }

    final timestamp = FieldValue.serverTimestamp();
    final batch = _db.batch();
    final uniqueInvitees = inviteeUids.toSet();
    ...
    for (final inviteeUid in uniqueInvitees) {
      ...
      batch.set(notificationRef, <String, dynamic>{
        ...
        'message':
            '$fromName invited you to join ${gymClass.title} on ${gymClass.dateText} at ${gymClass.timeText}.',
        'type': 'group_invite',
        'is_read': false,
        'from_uid': fromUid,
        'from_name': fromName,
    ...
    try {
      return await _db.runTransaction<GroupReservationResult>((transaction) async {
        ...
        transaction.update(classRef, <String, dynamic>{
          'filled': occupiedCount + assignedMats.length,
          'registeredCount': occupiedCount + assignedMats.length,
          'status': occupiedCount + assignedMats.length >= capacity
  ...
  static Future<String?> acceptGroupInvite({
    ...
    try {
      ...
      final existingRegistration = await _userRegistrationRef(userId, classId)
          .get();
  ...
  static String _friendlyFirestoreError(FirebaseException e) {
    switch (e.code) {
      case 'permission-denied':
        return 'You do not have permission to complete this action. (${e.code})';
      case 'unavailable':
        return 'Service is temporarily unavailable. Please try again. (${e.code})';
      case 'aborted':
        return 'This request conflicted with another update. Please try again. (${e.code})';
      case 'failed-precondition':
        return 'A required Firestore index or rule is missing for this request. (${e.code})';
      default:
        final message = (e.message ?? 'Unable to complete request right now.').trim();
